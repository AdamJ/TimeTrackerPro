...
- **Client persistence (`persistClients()`)**: to minimize Supabase calls, clients are **not** part of `forceSyncToDatabase`'s bulk save. Instead, the three client mutations keep a `clientsRef` in sync synchronously, and consumers call `persistClients()` (a stable callback that saves `clientsRef.current` via `dataService.saveClients`) only when a client actually changes. `SupabaseService.getClients`/`saveClients` are cached (`getCachedClients`/`setCachedClients` in `src/lib/supabase.ts`), so a load is a single read on cache miss and free thereafter.
