...
export class LocalStorageService implements DataService {
  ...
  async getCurrentDay(): Promise<CurrentDayData | null> {
    try {
      ...
      if (data._v !== SCHEMA_VERSION) {
        console.warn(
          'localStorage current day schema mismatch — clearing stale data'
        );
  ...
  async getArchivedDays(): Promise<DayRecord[]> {
    try {
      ...
      const data: DayRecord[] | undefined = Array.isArray(parsed)
        ? parsed
        : parsed?.days;
      if (!Array.isArray(parsed) && parsed?._v !== SCHEMA_VERSION) {
        console.warn(
          'localStorage archived days schema mismatch — clearing stale data'
        );
  ...
  async getProjects(): Promise<Project[]> {
    try {
      ...
      if (!Array.isArray(parsed) && parsed?._v !== SCHEMA_VERSION) {
        console.warn(
          'localStorage projects schema mismatch — clearing stale data'
        );
  ...
  async getCategories(): Promise<TaskCategory[]> {
    try {
      ...
      const parsed = JSON.parse(saved);
      // Support legacy bare array and new versioned format { data, _v }
      const data: TaskCategory[] = Array.isArray(parsed)
        ? parsed
        : parsed?.data;
      if (!Array.isArray(parsed) && parsed?._v !== SCHEMA_VERSION) {
        console.warn(
          'localStorage categories schema mismatch — clearing stale data'
        );
  ...
  async getTodos(): Promise<TodoItem[]> {
    try {
      ...
      if (!Array.isArray(parsed) && parsed?._v !== SCHEMA_VERSION) {
        console.warn(
          'localStorage todos schema mismatch — clearing stale data'
        );
